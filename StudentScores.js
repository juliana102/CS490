  ...
  if (httpRequest.status === 200) {
   ...
   for (x in response) {
     ...
     for (y in deductions) {
       var graderComments="";
       if (deductions[y]=="no") {
         graderComments="You received no deductions, good job!";
       }
      else if (deductions[y]=="for"){
        var deduction = Math.floor(studentScore/.80 -  studentScore);
        deductionsAccountedFor += deduction;
        graderComments=graderComments+"<br>You were missing a for loop. Points lost: " +deduction+"<br>";
      }
      else if (deductions[y]=="while"){
        var deduction = Math.floor(studentScore/.80 -  studentScore);
        deductionsAccountedFor += deduction;
        graderComments=graderComments+"<br>You were missing a while loop. Points lost: "+deduction+"<br>";
      }
      else if (deductions[y]=="if"){
        var deduction = Math.floor(studentScore/.80 -  studentScore);
        deductionsAccountedFor += deduction;
        graderComments=graderComments+"<br>You were missing an if statement. Points lost: "+deduction+"<br>";
      }
      else {
        var testCaseDeduction = pointsLost - deductionsAccountedFor;
        graderComments=graderComments+"<br>Points were deducted for failing test case. Number of test cases failed: "+deductions[y]+"<br>Points lost for failing Test Cases: "+testCaseDeduction;
      }
     }
     ...
     var scoreRow="<tr><td colspan='2'>Your points for question "+questionID+": "+studentScore+" out of "+questionPoints+"</td></tr>";
     var commentText="<tr><td>Professor Comment on Question "+questionID+" below: </td><td>Auto Grader Comments:</td></tr>";
